...
export default class CategoriaController{
    ...
    static buscarPorNome(req, res){
        try {
            ...
            if(categorias.lenght === 0){
                res.status(404).json({msg: "categoria não encontrado com essa descricao!"});
                return;
            }
            res.status(200).json({categorias})
        } catch (error) {
    ...
    static criar(req, res){
        try {
            ...
            res.status(200).json({msg: "categoria Criado", categoria: novo})
        } catch (error) {
                res.status(500).json({msg: "Erro ao tentar criar categoria", erro: error.message}) 
        }
    }
    static atualizar(req, res){
        try {
            ...
            res.status(201).json({msg: "categoria Atualizado com sucesso", categoria: atualizado})
        } catch (error) {
                res.status(500).json({msg: "Erro ao tentar criar categoria", erro: error.message}) 
        }
}
    static deletar(req, res){
        try {
            ...
            res.status(200).json({msg: "categoria excluído."})
        } catch (error) {
            res.status(500).json({msg: "Erro ao tentar tentar apagar categoria.", erro: error.message})
        }
    }
}