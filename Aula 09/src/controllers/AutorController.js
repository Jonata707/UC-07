...
export default class AutorController{
    ...
    static buscarPorNacionalidade(req, res){
        try {
            ...
            if(autores.lenght === 0){
                res.status(404).json({msg: "Autor não encontrado com essa nacionalidade!"});
                return;
            }
            res.status(200).json({autores})
        } catch (error) {
    ...
    static criar(req, res){
        try {
            ...
            res.status(200).json({msg: "Autor Criado", autor: novo})
        } catch (error) {
                res.status(500).json({msg: "Erro ao tentar criar autor", erro: error.message}) 
        }
    }
    static atualizar(req, res){
        try {
            ...
            res.status(201).json({msg: "Autor Atualizado com sucesso", autor: atualizado})
        } catch (error) {
                res.status(500).json({msg: "Erro ao tentar criar autor", erro: error.message}) 
        }
}
    static deletar(req, res){
        try {
            ...
            res.status(500).json({msg: "Erro ao tentar tentar apagar autor.", erro: error.message})
        }
    }
}